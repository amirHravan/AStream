#!/bin/bash
# Scenario 4: Long, Proxy
# Usage: ./vm_client_scen4.sh
...
echo "Running Scenario 4 (Long, Proxy) against $SERVER_IP:$SERVER_PORT via Proxy $PROXY_IP:$PROXY_PORT..."
python3 dash_client.py -m "http://$SERVER_IP:$SERVER_PORT/media/mpd/long/manifest.mpd" -p "basic" --use-pep --pep-host "$PROXY_IP" --pep-port "$PROXY_PORT"
