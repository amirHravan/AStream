...
def weighted_dash(
    bitrates, dash_player, weighted_dwn_rate, curr_bitrate, next_segment_sizes
):
    """
    ...
    config_dash.LOG.debug(
        "Buffer_length = {} Initial Buffer = {} Available video = {} seconds, alpha = {}. "
        "Beta = {} WDR = {}, curr Rate = {}".format(
    ...
    # switch to a lower suitable bitrate
        ...
        float(next_segment_sizes[curr_bitrate]) / weighted_dwn_rate
        > available_video_duration
    ):
        config_dash.LOG.debug(
            "next_segment_sizes[curr_bitrate]) weighted_dwn_rate > available_video"
        )
        for bitrate in reversed(bitrates):
            if bitrate < curr_bitrate:
                if (
                    float(next_segment_sizes[bitrate]) / weighted_dwn_rate
                    < available_video_duration
                ):
        ...
        if curr_bitrate >= max(bitrates):
            ...
            config_dash.LOG.info(
                "next_segment_sizes[higher_bitrate] = {}, weighted_dwn_rate = {} , "
                "available_video={} seconds, ratio = {}".format(
                    next_segment_sizes[higher_bitrate],
                    weighted_dwn_rate,
                    available_video_duration,
