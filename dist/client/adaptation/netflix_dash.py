...
def get_rate_map(bitrates):
    ...
    try:
        ...
        if intermediate_levels:
            marker_length = (
                config_dash.NETFLIX_CUSHION - config_dash.NETFLIX_RESERVOIR
...
def netflix_dash(
    ...
    try:
        ...
        available_video_segments = max(
            0, dash_player.buffer.qsize() - dash_player.initial_buffer
        )
        ...
        if state == "INITIAL":
            ...
            if curr_bitrate not in bitrates:
                config_dash.LOG.warning(
                    f"Current bitrate {curr_bitrate} not in available bitrates"
                )
                return bitrates[0], rate_map, state
            ...
            try:
                ...
                if segment_download_rate > 0:
                    delta_B = (
                        dash_player.segment_duration
                        - average_segment_sizes[curr_bitrate] / segment_download_rate
                    )
                else:
                    delta_B = 0
                ...
                if (
                    delta_B
                    > config_dash.NETFLIX_INITIAL_FACTOR * dash_player.segment_duration
                    and current_index < len(bitrates) - 1
                ):
...
def get_rate_netflix(
    bitrates,
    current_buffer_occupancy,
    buffer_size=config_dash.NETFLIX_BUFFER_SIZE,
    rate_map=None,
):
    """
    ...
    try:
        ...
        try:
            buffer_percentage = (
                current_buffer_occupancy / buffer_size if buffer_size else 0
            )
