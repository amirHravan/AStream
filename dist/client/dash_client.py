...
import dash_buffer
import read_mpd
from adaptation import basic_dash, basic_dash2, netflix_dash, weighted_dash
    ...
    try:
        ...
        if segment_size > 0 and download_duration > 0:
            ...
            config_dash.LOG.error(
                f"Download completed but file size is 0 or duration is 0: {segment_url}"
            )
            return None
...
def get_media_all(domain, media_info, file_identifier, done_queue):
    ...
    try:
        ...
        config_dash.LOG.info(
            f"Processing {len(segments)} segments for bandwidth {bandwidth}"
        )
...
def start_playback_smart(
    ...
    dash_player = dash_buffer.DashPlayer(
        dp_object.playback_duration, video_segment_duration
    )
    dash_player.start()
    ...
    for bitrate in dp_object.video:
        ...
        if "$Bandwidth$" in dp_object.video[bitrate].initialization:
            dp_object.video[bitrate].initialization = dp_object.video[
                bitrate
            ].initialization.replace("$Bandwidth$", str(bitrate))
        media_urls = [dp_object.video[bitrate].initialization] + dp_object.video[
            bitrate
        ].url_list
        # print "media urls"
        # print media_urls
    ...
    for segment_number, segment in enumerate(
        ...
        print(
            "dp_object.video[bitrate].start={}".format(dp_object.video[bitrate].start)
        )
            ...
            if playback_type.upper() == "BASIC":
                ...
                if segment_number < len(dp_list) - 1 + dp_object.video[bitrate].start:
                    try:
                        ...
                        current_bitrate, netflix_rate_map, netflix_state = (
                            netflix_dash.netflix_dash(
                                bitrates,
                                dash_player,
                                segment_download_rate,
                                current_bitrate,
                                average_segment_sizes,
                                netflix_rate_map,
                                netflix_state,
                            )
                        )
                        config_dash.LOG.info(
                            "NETFLIX: Next bitrate = {}".format(current_bitrate)
                        )
                    except IndexError as e:
                        config_dash.LOG.error(e)
                else:
                ...
                if dash_player.buffer.qsize() >= config_dash.NETFLIX_BUFFER_SIZE:
                    delay = (
                        dash_player.buffer.qsize() - config_dash.NETFLIX_BUFFER_SIZE + 1
                    ) * segment_duration
                    config_dash.LOG.info("NETFLIX: delay = {} seconds".format(delay))
            else:
                config_dash.LOG.error(
                    "Unknown playback type:{}. Continuing with basic playback".format(
                        playback_type
                    )
                )
        ...
        # print segment_url
        config_dash.LOG.info(
            "{}: Segment URL = {}".format(playback_type.upper(), segment_url)
        )
        if delay:
            delay_start = time.time()
            config_dash.LOG.info(
                "SLEEPING for {}seconds ".format(delay * segment_duration)
            )
            while time.time() - delay_start < (delay * segment_duration):
                time.sleep(1)
            delay = 0
            config_dash.LOG.debug(
                "SLEPT for {}seconds ".format(time.time() - delay_start)
            )
        start_time = timeit.default_timer()
        try:
            # print 'url'
            ...
            if result:
                ...
                config_dash.LOG.info(
                    "{}: Downloaded segment {}".format(
                        playback_type.upper(), segment_url
                    )
                )
            else:
                config_dash.LOG.error(
                    "Failed to download segment {}".format(segment_url)
                )
                continue
        except IOError as e:
        ...
        config_dash.JSON_HANDLE["segment_info"].append(
            (segment_name, current_bitrate, segment_size, segment_download_time)
        )
        ...
        if playback_type.upper() == "SMART" and weighted_mean_object:
            weighted_mean_object.update_weighted_mean(
...
def get_segment_sizes(dp_object, segment_number):
            ...
            if hasattr(media_object, "segment_sizes") and media_object.segment_sizes:
                try:
                    ...
                    if 0 <= segment_number < len(media_object.segment_sizes):
                        segment_sizes[bitrate] = media_object.segment_sizes[
                            segment_number
                        ]
                        continue
                except (IndexError, TypeError) as e:
                    config_dash.LOG.debug(
                        f"Could not get size for segment {segment_number} at bitrate {bitrate}: {e}"
                    )

            # Fallback: Estimate size based on bitrate and segment duration
            # Size = (bitrate * segment_duration) / 8 to convert bits to bytes
            if (
                hasattr(media_object, "segment_duration")
                and media_object.segment_duration
            ):
                estimated_size = (bitrate * media_object.segment_duration) / 8
                segment_sizes[bitrate] = estimated_size
                config_dash.LOG.debug(
                    f"Using estimated size for bitrate {bitrate}: {estimated_size} bytes"
                )
            else:
                ...
                segment_sizes[bitrate] = estimated_size
                config_dash.LOG.debug(
                    f"Using default estimated size for bitrate {bitrate}: {estimated_size} bytes"
                )
    ...
    config_dash.LOG.debug(
        f"Segment sizes for segment {segment_number}: {segment_sizes}"
    )
    return segment_sizes
...
def get_average_segment_sizes(dp_object):
    ...
    config_dash.LOG.info(
        "The avearge segment size for is {}".format(average_segment_sizes.items())
    )
...
def clean_files(folder_path):
    ...
    if os.path.exists(folder_path):
        try:
            ...
            config_dash.LOG.info(
                "Unable to delete the folder {}. {}".format(folder_path, e)
            )
        config_dash.LOG.info(
...
def start_playback_all(dp_object, domain, video_segment_duration):
    ...
    for bitrate in dp_object.video:
        ...
        process = Process(
            target=get_media_all,
            args=(
                domain,
                (bitrate, dp_object.video),
                file_identifier,
                video_done_queue,
            ),
        )
    ...
    try:
        ...
        while count < len(dp_object.video):
            try:
                ...
                if status == "ERROR":
                    config_dash.LOG.error(
                        f"Error downloading bitrate {bitrate}: {info}"
                    )
                elif status == "STOP":
                    config_dash.LOG.info(
                        f"Completed download of {bitrate} in {info} seconds"
                    )
                    count += 1
...
def create_arguments(parser):
    ...
    parser.add_argument(
        "-l", "--LIST", action="store_true", help="List all the representations"
    )
    parser.add_argument(
        "-p",
        "--PLAYBACK",
    ...
    parser.add_argument(
        "-n", "--SEGMENT_LIMIT", default=SEGMENT_LIMIT, help="The Segment number limit"
    )
    parser.add_argument(
        "-d",
        "--DOWNLOAD",
    ...
    parser.add_argument(
        "--use-pep", action="store_true", help="Use Performance Enhancing Proxy"
    )
    parser.add_argument(
        "--pep-host", default="localhost", help="PEP proxy host address"
    )
    parser.add_argument("--pep-port", type=int, default=8888, help="PEP proxy port")
    parser.add_argument(
        "--buffer-size",
        type=int,
        default=2 * 1024 * 1024,
...
def main():
    ...
    try:
        ...
        config_dash.LOG.info(
            "The DASH media has %d video representations" % len(dp_object.video)
        )
        ...
        try:
            ...
            if "all" in args.PLAYBACK.lower():
                ...
                start_playback_smart(
                    dp_object,
                    domain,
                    "BASIC",
                    args.DOWNLOAD,
                    video_segment_duration,
                    use_pep=args.use_pep,
                    pep_host=args.pep_host,
                ...
                config_dash.LOG.error(
                    "Unknown Playback parameter {}".format(args.PLAYBACK)
                )
                return None
