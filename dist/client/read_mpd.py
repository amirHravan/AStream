...
def process_segment_info(
    segment_info: ET.Element, media_object: MediaObject, bandwidth: int, cut_url: str
) -> Optional[float]:
    """
            ...
            if "duration" in segment_info.attrib:
                ...
                for segment_URL in segment_info:
                    if "SegmentURL" in get_tag_name(segment_URL.tag or ""):
                        ...
                        try:
                            size_str = (
                                media.split("/")[0].split("_")[-1].split("kbit")[0]
                            )
                            segment_size = float(size_str) * SIZE_DICT.get(
                                "Kbits", 1024
                            )
...
def get_url_list(media_object, segment_duration, playback_duration, bitrate):
    ...
    try:
        ...
        media_object.initialization = (
            media_object.base_url_path + media_object.initialization
        )
        ...
        if num_segments <= 0:
            ...
            config_dash.LOG.warning(
                f"Invalid segment calculation, using default: {num_segments}"
            )
        ...
        if media_object.url_list:
            config_dash.LOG.info(
                f"Generated {len(media_object.url_list)} URLs for bitrate {bitrate}"
            )
...
def get_segment_duration(element: ET.Element, ns: dict) -> Optional[float]:
    ...
    try:
        ...
        template = (
            element.find(".//SegmentTemplate")
            if not ns
            else element.find(".//dash:SegmentTemplate", ns)
        )
        ...
        # Check SegmentList
        seg_list = (
            element.find(".//SegmentList")
            if not ns
            else element.find(".//dash:SegmentList", ns)
        )
        ...
        # Check SegmentBase
        seg_base = (
            element.find(".//SegmentBase")
            if not ns
            else element.find(".//dash:SegmentBase", ns)
        )
...
def read_mpd(
    mpd_file: str, dashplayback, mpd_url: str
    ...
    try:
        ...
        if duration_str:
            ...
            config_dash.LOG.info(
                f"Found media presentation duration: {dashplayback.playback_duration}s"
            )
            if "video_metadata" not in config_dash.JSON_HANDLE:
                config_dash.JSON_HANDLE["video_metadata"] = {}
            config_dash.JSON_HANDLE["video_metadata"]["playback_duration"] = (
                dashplayback.playback_duration
            )
        else:
            ...
            if "video" in mime_type or content_type == "video":
                ...
                if segment_template is not None:
                    ...
                    if duration_str:
                        try:
                            ...
                            config_dash.LOG.info(
                                f"Found segment duration: {video_segment_duration}s"
                            )
                        except (ValueError, TypeError) as e:
                            config_dash.LOG.error(
                                f"Error calculating segment duration: {e}"
                            )
                ...
                for representation in adaptation_set.findall(
                    ...
                    try:
                        ...
                        if (
                            bandwidth
                            not in config_dash.JSON_HANDLE["video_metadata"][
                                "available_bitrates"
                            ]
                        ):
                            config_dash.JSON_HANDLE["video_metadata"][
                        ...
                        if segment_template is not None:
                            media_object.start = int(
                                segment_template.get("startNumber", "1")
                            )
        ...
        config_dash.LOG.info(
            f"Successfully parsed MPD with {len(dashplayback.video)} video representations"
        )
