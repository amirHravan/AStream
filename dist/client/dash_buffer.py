...
class DashPlayer:
    ...
    def initialize_player(self):
        ...
        config_dash.LOG.info(
            "Initialized player with video length {}".format(self.playback_duration)
        )
        while True:
            # Video stopped by the user
            if self.playback_state == "END":
                config_dash.LOG.info(
                    "Finished playback of the video: {} seconds of video played for {} seconds".format(
            ...
            if self.playback_state == "STOP":
                ...
                config_dash.LOG.info(
                    "Player Stopped at time {}".format(time.time() - start_time)
                )
                config_dash.JSON_HANDLE["playback_info"]["end_time"] = time.time()
                self.playback_timer.pause()
            ...
            if self.playback_state == "PAUSE":
                if not paused:
                    ...
                    config_dash.LOG.info(
                        "Player Paused after {:4.2f} seconds of playback".format(
            ...
            if self.playback_state == "BUFFERING":
                if not buffering:
                    config_dash.LOG.info(
                        "Entering buffering stage after {} seconds of playback".format(
                            self.playback_timer.time()
                        )
                    )
                    self.playback_timer.pause()
                ...
                # If the size of the buffer is greater than the RE_BUFFERING_DURATION then start playback
                    ...
                    remaining_playback_time = (
                        self.playback_duration - self.playback_timer.time()
                    )
                    if (self.buffer.qsize() >= config_dash.RE_BUFFERING_COUNT) or (
                        ...
                        if interruption_start:
                            ...
                            config_dash.JSON_HANDLE["playback_info"]["interruptions"][
                                "events"
                            ].append((interruption_start, interruption_end))
                            config_dash.JSON_HANDLE["playback_info"]["interruptions"][
                                "total_duration"
                            ] += interruption
                            config_dash.LOG.info(
                                "Duration of interruption = {}".format(interruption)
                            )
                            interruption_start = None
                        self.set_state("PLAY")
            ...
            if self.playback_state == "PLAY":
                ...
                config_dash.LOG.info(
                    "Reading the segment number {} from the buffer at playtime {}".format(
                ...
                while self.playback_timer.time() < future:
                    ...
                    if not self.playback_start_time:
                        ...
                        config_dash.LOG.info(
                            "Started playing with representation {} at {}".format(
                    ...
                    config_dash.LOG.debug(
                        "Decrementing buffer_length by {}. dash_buffer = {}".format(
                ...
                if self.segment_limit:
                    if int(play_segment["segment_number"]) >= self.segment_limit:
                        self.set_state("STOP")
                        config_dash.LOG.info(
                            "Stopped playback after segment {} at playtime {}".format(
    ...
    def write(self, segment):
        ...
        if not self.actual_start_time:
            ...
            config_dash.JSON_HANDLE["playback_info"]["start_time"] = (
                self.actual_start_time
            )
        config_dash.LOG.info(
            "Writing segment {} at time {}".format(
