...
class DASHDownloader:
    ...
    def __init__(
        ...
        if mode == DownloadMode.PEP and pep_host and pep_port:
            self.pep_downloader = PEPDownloader(
                max_buffer_size=max_buffer_size, pep_host=pep_host, pep_port=pep_port
            )
    ...
    def _log_download_stats(
        self, segment_url: str, segment_size: int, download_duration: float
    ) -> None:
    ...
    def _download_segment_direct(
        self, segment_url: str, dash_folder: str
    ) -> Optional[Tuple[int, str]]:
        """Direct download implementation"""
        try:
            config_dash.LOG.debug(f"Direct download: {segment_url}")
            segment_path, segment_filename = self._prepare_download(
                segment_url, dash_folder
            )
            ...
            config_dash.LOG.error(
                f"URL Error downloading {segment_url}: {error.reason}"
            )
            return None
...
def init_downloader(
    ...
    try:
        ...
        config_dash.LOG.warning(
            f"Invalid mode '{mode}', falling back to direct download"
        )
