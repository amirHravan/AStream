...
def configure_ping_options(ping_packets=config_dash.PING_PACKETS):
    ...
    if system.startswith("linux"):
        config_dash.ping_option_nb_pkts = " ".join(
            (PING_OPTION_LINUX, str(ping_packets))
        )
    ...
    elif system.startswith("microsoft") or system.startswith("windows"):
        config_dash.ping_option_nb_pkts = " ".join(
            (PING_OPTION_WINDOWS, str(ping_packets))
        )
        ...
        config_dash.ping_option_nb_pkts = " ".join(
            (PING_OPTION_DARWIN, str(ping_packets))
        )
        ...
        config_dash.LOG.warn(
            "Ping option is not known on your system: %s" % config_dash.SYSTEM
        )
        return None
...
def ping_ip(ip_address, ping_packets=config_dash.PING_PACKETS, verbose=False):
    ...
    if rtt_times:
        ...
        rtt_values = [
            rtt_values[config_dash.index_rtt_min],
            rtt_values[config_dash.index_rtt_avg],
            rtt_values[config_dash.index_rtt_max],
        ]

        config_dash.LOG.debug("RTT stats found for ip: %s" % ip_address)
        return map(float, rtt_values)
    config_dash.LOG.error("The ping returned an unexpected RTT fomat: %s" % rtt_stats)
    return None
